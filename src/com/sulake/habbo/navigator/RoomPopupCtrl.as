package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_101:GuestRoomData;
      
      private var var_2044:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2044 = new RoomThumbnailRenderer(param1);
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_158 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_158.visible = true;
         Util.hideChildren(this.var_158);
         this.refreshRoomName(this.var_158,this.var_101);
         if(this.var_101.ownerName != "" && this.var_101.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_158,!!this.var_101.event ? this.getEventCtg(this.var_101.categoryId) : this.getRoomCtg(this.var_101.categoryId));
         this.refreshRoomDesc(this.var_158,this.var_101);
         this.refreshExtraCont();
         this.refreshInfo(this.var_158,"trading_allowed",this.var_101.allowTrading);
         this.refreshInfo(this.var_158,"doormode_doorbell",this.var_101.doorMode == 1);
         this.refreshInfo(this.var_158,"doormode_password",this.var_101.doorMode == 2);
         Util.moveChildrenToColumn(this.var_158,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_158.height = Util.getLowestPoint(this.var_158);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_158,this.var_101.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_158.findChildByName("roomowner"));
         if(this.var_101.ownerName != "" && this.var_101.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_158.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1141.refreshTags(_loc1_,this.var_101.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_101.score);
         if(this.var_101.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_101.eventCreationTime);
         }
         this.var_2044.refreshThumbnail(_loc1_,this.var_101.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_214,Tabs.const_242,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
