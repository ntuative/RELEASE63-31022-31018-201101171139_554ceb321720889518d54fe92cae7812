package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1812 = param1.type;
            this.var_1402 = false;
            this.var_1147 = param1.position;
         }
         else
         {
            this.var_1402 = true;
         }
         this.var_2573 = param1.hasHabboClub;
         this.var_1811 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1147.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1147.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1147.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1147.toString());
         this.showInterface();
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(this.var_1812)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_378:
                  _loc1_.caption = !!this.var_1811 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1402;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_500:
                  _loc1_.caption = !!this.var_1811 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1402;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_2573;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_627);
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1812 == RoomWidgetRoomQueueUpdateEvent.const_378)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_673);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
