package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      public function EffectView()
      {
         ...
         this.var_188.addEventListener(TimerEvent.TIMER,this.onUpdate);
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_188.stop();
            this.var_188.removeEventListener(TimerEvent.TIMER,this.onUpdate);
            this.var_188 = null;
         }
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_165.isActive)
            {
               _loc2_ = this.var_165.secondsLeft / Number(this.var_165.duration);
               this.var_868.width = _loc2_ * this.var_350;
            }
            else
            {
               this.var_868.width = 0;
               this.var_188.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_867.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_867.text;
            if(this.var_165.isActive)
            {
               _loc2_ = this.var_165.secondsLeft;
               ...
               this.var_867.text = _loc1_;
            }
            else
            {
               this.var_867.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_165.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_165.isActive)
         {
         ...
         else
         {
            ...
            this.var_866 = this._window.findChildByName("effect_hilite");
            this.var_866.visible = false;
         }
         this.var_867 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_868 = this._window.findChildByName("loader_bar");
         if(this.var_868 != null)
         {
            this.var_350 = this.var_868.width;
            this.var_188.start();
            this.onUpdate();
         }
         if(this.var_165.icon)
         {
            this.setElementBitmap("effect_icon",this.var_165.icon);
         }
      }
