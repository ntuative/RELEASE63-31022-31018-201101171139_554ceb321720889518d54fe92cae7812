package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_485.numGridItems)
         {
            return;
         }
         this.select(this.var_485.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_485 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         {
            _loc3_ = param1.buildFromXML(this.var_1729) as IWindowContainer;
            ...
            _loc3_.width = this.var_579.width;
            _loc3_.height = this.var_579.height;
            this.var_485.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_579.width,this.var_579.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_579,this.var_579.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1114.width,this.var_1114.height,true,0);
      ...
      {
         var _loc2_:int = this.var_485.getGridItemIndex(param1.target as IWindow);
