package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private static const const_1628:int = 60000;
      ...
      private var var_82:AvatarCanvas;
      
      private var _disposed:Boolean;
      
      private var _bodyPartCache:AvatarImageBodyPartCache;
      
      private var var_322:IActiveActionData;
      ...
      private var var_163:BitmapData;
      ...
      private var var_2211:ByteArray;
      
      private var var_2212:ImageData;
      
      private var var_1606:PaletteMap;
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_687 == GeometryType.const_1064 && param1 == GeometryType.const_1143 || this.var_687 == GeometryType.const_1143 && param1 == GeometryType.const_1064)
         {
            this.var_687 = param1;
            this.var_82 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
         ...
         this.var_322 = this._bodyPartCache.getAction();
         var _loc5_:IActiveActionData = this.var_322;
         var _loc6_:* = [];
         var _loc7_:Dictionary = new Dictionary();
         var _loc8_:Point = new Point();
         if(!(!this.var_322 || !this.var_322.definition))
         {
            if(this.var_322.definition.isAnimation)
            {
               _loc11_ = _loc3_;
               _loc12_ = this._structure.getAnimation(this.var_322.definition.state + "." + this.var_322.actionParameter);
               _loc13_ = param2 - this.var_322.startFrame;
               if(_loc12_ != null)
               {
                  _loc14_ = _loc12_.getLayerData(_loc13_,param1,this.var_322.overridingAction);
                  if(_loc14_ != null)
                  {
                     ...
                     {
                        this.var_322 = _loc14_.action;
                     }
                     if(_loc14_.type == AnimationLayerData.const_600)
                     {
                        if(_loc14_.action != null)
                        {
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1394.getFigure(),_loc5_,this.var_687,_loc3_,_loc6_,_loc7_);
         ...
         if(this.var_387 == null)
         {
            ...
            this.var_387 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_322);
            if(this.var_387 == null)
            {
               return null;
            }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_82)
         {
            this.var_82 = this._structure.getCanvas(this._scale,this.var_687);
            if(!this.var_82)
            {
               return null;
            }
            this.var_2210 = new Point(0,this.var_82.height - int(this.var_82.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1574[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_776 || _loc10_ == AvatarAction.const_364;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1282 = this.var_107.bodyPartId;
               this.var_162 = this.var_107.partType;
               this.var_1605 = this.var_107.partId;
               this._frame = this.var_107.getFrameIndex(param3);
               _loc5_ = param1;
               _loc7_ = false;
               if(_loc6_)
               {
                  if(_loc9_ == "wav" && (this.var_162 == "lh" || this.var_162 == "ls" || this.var_162 == "lc" || this.var_162 == "lhs"))
                  {
                     _loc7_ = true;
                  }
                  else if(_loc9_ == "drk" && (this.var_162 == "rh" || this.var_162 == "rs" || this.var_162 == "rc" || this.var_162 == "rhs"))
                  {
                     _loc7_ = true;
                  }
                  else if(_loc9_ == "crr" && this.var_162 == "rhs")
                  {
                     _loc7_ = true;
                  }
                  else if(this.var_162 == "ri")
                  {
                     _loc7_ = true;
                  }
                  else if(this.var_162 == "cp")
                  {
                     _loc7_ = true;
                  }
                  else
                  {
                     ...
                     if(this.var_107.flippedPartType != this.var_162)
                     {
                        this.var_162 = this.var_107.flippedPartType;
                     }
                  }
               }
               this.var_1283 = this._scale + "_" + this.var_107.action.assetPartDefinition + "_" + this.var_162 + "_" + this.var_1605 + "_" + _loc5_ + "_" + this._frame;
               this.var_383 = this._assets.getAssetByName(this.var_1283) as BitmapDataAsset;
               if(this.var_383 == null)
               {
                  this.var_1283 = this._scale + "_std_" + this.var_162 + "_" + this.var_1605 + "_" + _loc5_ + "_0";
                  this.var_383 = this._assets.getAssetByName(this.var_1283) as BitmapDataAsset;
               }
               if(!(!this.var_383 || !this.var_383.content))
               {
                  if(this.var_107.paletteMapId > -1)
                  {
                     this.var_1606 = this._structure.renderManager.paletteManager.getPalette(this.var_107.paletteMapId);
                     if(this.var_1606 != null)
                     {
                        this.var_834 = this.var_383.content as BitmapData;
                        this.var_163 = new BitmapData(this.var_834.width,this.var_834.height);
                        this.var_2211 = this.var_1606.colorize(this.var_834.getPixels(this.var_834.rect));
                        this.var_163.setPixels(this.var_163.rect,this.var_2211);
                     }
                     else
                     {
                        this.var_163 = (this.var_383.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1282 == AvatarBodyPartType.const_220 || this.var_1282 == AvatarBodyPartType.const_1387 || this.var_1282 == AvatarBodyPartType.const_1386) && this.var_107.color != null)
                  {
                     _loc16_ = 0.7;
                     ...
                     this.var_163.colorTransform(this.var_163.rect,_loc18_);
                  }
                  else if(this.var_107.isColorable && this.var_107.color != null)
                  {
                     this.var_163.colorTransform(this.var_163.rect,this.var_107.color.colorTransform);
                  }
                  if(this.var_107.isBlendable)
                  {
                     this.var_163.colorTransform(this.var_163.rect,this.var_107.blendTransform);
                  }
                  this.var_833 = this.var_383.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_48)
                     {
                  ...
                  this.var_2212 = new ImageData(this.var_163,this.var_833,_loc7_);
                  this.var_835.push(this.var_2212);
               }
            }
            _loc8_--;
         }
         ...
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2210.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_48)
            {
