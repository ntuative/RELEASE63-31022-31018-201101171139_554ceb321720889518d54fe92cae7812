package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_874:int = 3;
      
      private var var_388:AssetLoaderStruct;
      
      private var var_875:Timer;
      ...
      private const const_1918:int = 100;
      
      private const const_1530:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_388 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_875 = new Timer(this.const_1918,1);
         this.var_875.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_388)
         {
            this.var_388.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_388.addEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_388)
         {
            this.var_388.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_388.removeEventListener(AssetLoaderEvent.const_43,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_874);
            if(this.var_1328.indexOf("?") > 0)
            {
               _loc2_ = this.var_1328 + "&retry=" + this.var_874;
            }
            else
            {
               _loc2_ = this.var_1328 + "?retry=" + this.var_874;
            ...
            this.var_388 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1670 = true;
         for each(_loc2_ in this.var_1091)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2358,this._assets);
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1670)
         {
            this.var_1091.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_496[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_496[_loc3_] = _loc6_;
            }
      ...
      {
         var _loc1_:* = null;
         while(this.var_709.length > 0 && this.var_710.length < this.const_1530)
         {
