package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            if(_loc8_ && _loc8_.avatarRenderManager && _loc8_.avatarRenderManager.petDataManager)
            {
               ...
               if(_loc9_)
               {
                  this.var_558 = _loc9_.sellableBreeds;
                  this.var_321 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_558.length)
         {
            _loc10_ = this.var_558[_loc6_] as IBreed;
         ...
         events.addEventListener(WidgetEvent.const_1134,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_321)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_464 = param1.index;
         if(this.var_464 < 0 || this.var_464 > this.var_321.length)
         {
            this.var_464 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_464 >= 0 && this.var_464 < this.var_321.length)
         {
            _loc7_ = this.var_321[this.var_464] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_558[this.var_1601] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_829,_loc3_.id,_loc2_,AvatarScaleType.const_48,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_302,AvatarAction.const_762);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_464 >= this.var_321.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_321[this.var_464] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_558[this.var_1601] as IBreed;
