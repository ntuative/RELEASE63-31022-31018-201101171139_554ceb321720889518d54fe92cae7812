package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private static const const_1212:String = "poster";
       
      
      public const const_1791:int = 1;
      
      public const const_1726:int = 2;
      
      public const const_1947:int = 3;
      ...
      public function dispose() : void
      {
         ...
         if(this.var_169)
         {
            for each(_loc1_ in this.var_169)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
      ...
      {
         this.requestOffers(this.var_2581,this.var_2582,this.var_2553,this.var_2583);
      }
      ...
      {
         if(!this.var_169 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_169.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1791,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            ...
            if(_loc3_.status == MarketPlaceOfferState.const_445)
            {
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_169)
         {
            for each(_loc5_ in this.var_169)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_169.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         this.var_2511 = _loc3_.creditsWaiting;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_169.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_169.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_169.add(parser.offerId,updateItem);
            }
            this.var_169.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1726,updateItem);
            this._visualization.listUpdatedNotify();
         }
      ...
      {
         if(param1.furniCategoryId != this.var_2524 || param1.furniTypeId != this.var_2523)
         {
            return;
         }
      ...
      private function isPosterItem(param1:IMarketPlaceOfferData) : Boolean
      {
         ...
         if(param1.furniType == 2 && param1.stuffData != null)
         {
            ...
            {
               _loc4_ = _loc3_.name;
               if(_loc4_ != null && _loc4_ == const_1212)
               {
                  _loc2_ = true;
               }
            }
