package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1267:uint = 1;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_1922 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            _loc4_ = this.var_247[param1];
            if(_loc4_)
            {
               _loc5_ = 0;
               for each(_loc6_ in _loc4_)
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1922 == param3)
                  {
      ...
      {
         return !!this._disposed ? false : this.var_247[param1] != null;
      }
      
      public function callEventListeners(param1:String) : void
      {
         var _loc3_:* = null;
         var _loc2_:Array = this.var_247[param1];
         if(_loc2_)
         {
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = this.var_247[event.type];
         if(array)
         {
            callbacks = [];
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1267) : uint(const_578);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            {
               _loc2_ = this.var_247[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete this.var_247[_loc1_];
            }
            this.var_247 = null;
